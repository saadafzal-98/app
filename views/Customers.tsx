...
const Customers: React.FC<CustomersProps> = ({ onCustomerSelect }) => {
  ...
  const handleOpenAdd = async () => {
    ...
    setFormData({
      name: '',
      phone: '',
      supplyRate: settings?.defaultSupplyRate ?? 10,
      openingBalance: 0
    });
    setErrors({});
    setShowModal('add');
  ...
  const validate = async (isEdit: boolean) => {
    ...
    if (formData.supplyRate < 0) newErrors.supplyRate = "Cannot be negative";
    ...
    <div className="space-y-6 page-transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCustomers.map(customer => (
          ...
          >
            ...
            <div className="absolute top-0 right-0 w-12 h-12 bg-emerald-500/5 rotate-45 translate-x-6 -translate-y-6 group-hover:bg-emerald-500/10 transition-colors"></div>
          </div>
        ))}
      </div>
      ...
      {filteredCustomers.length === 0 && !loading && (
        <div className="text-center py-24 opacity-30">
          ...
          <p className="text-sm font-black uppercase tracking-widest">No active route profiles found</p>
        </div>
      )}
