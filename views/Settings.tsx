...
const Settings: React.FC<SettingsProps> = ({ darkMode, setDarkMode, onSecurityUpdate }) => {
  ...
  // Use strings for input management
  ...
  useEffect(() => {
    db.settings.get('global').then(s => {
      if (s) {
        ...
        setFarmRate((s.farmRate || 150).toString());
        setDefaultSupplyRate((s.defaultSupplyRate || 10).toString());
      }
    });
  }, []);
  ...
  const handleSaveBusiness = async () => {
    await db.settings.update('global', { 
      farmRate: parseFloat(farmRate) || 0, 
      defaultSupplyRate: parseFloat(defaultSupplyRate) || 0 
    });
    setStatus({ type: 'success', msg: 'Business rates updated!' });
    setTimeout(() => setStatus(null), 3000);
  };

  const handleNumericInput = (field: 'farmRate' | 'defaultSupplyRate', value: string) => {
    const sanitized = value.replace(/[^0-9.]/g, '');
    const noLeadingZeros = sanitized.replace(/^0+(?=\d)/, '');
    if (field === 'farmRate') setFarmRate(noLeadingZeros);
    else setDefaultSupplyRate(noLeadingZeros);
  };
          ...
          <div className="flex flex-col space-y-2">
            ...
            <div className="relative">
              ...
              <input 
                ...
                value={farmRate}
                onChange={(e) => handleNumericInput('farmRate', e.target.value)}
              />
            </div>
          </div>
          <div className="flex flex-col space-y-2">
            ...
            <div className="relative">
              ...
              <input 
                ...
                onChange={(e) => handleNumericInput('defaultSupplyRate', e.target.value)}
              />
            </div>
          </div>
        </div>
